...
void PlaneModel::privCreateVAO(const char * const pModelFileName)
{
	File::Handle fh2;
	File::Error  ferror;
	...
	// allocate buffers
	Vert_xyzuvn  *planeData = new Vert_xyzuvn[(unsigned int)modelHdr.numVerts];
	...
	ferror = File::Read(fh2, planeData, sizeof(Vert_xyzuvn) * this->numVerts);
	assert(ferror == File::Error::SUCCESS);
	...
	glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)(sizeof(Vert_xyzuvn) * this->numVerts), planeData, GL_STATIC_DRAW);

	// Define an array of generic vertex attribute data

	// Vert data is location: 0  (used in vertex shader)
	void *offsetVert = (void *)((unsigned int)&planeData[0].x - (unsigned int)planeData);
	...
	void *offsetTex = (void *)((unsigned int)&planeData[0].u - (unsigned int)planeData);
	...
	void *offsetNorm = (void *)((unsigned int)&planeData[0].nx - (unsigned int)planeData);
...
PlaneModel::PlaneModel(const char * const pModelFileName)
	: Model()
{
